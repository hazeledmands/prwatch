...
- **project context.** most editors get the file and nothing else. an editor whose CLI documents opening several paths at once — where one can be a directory and another a file with a position — also gets the repo root, ahead of the file, so the file opens inside the project instead of attaching to whichever window happened to be focused last. `zed` is the case this exists for: its argument is a list of `path:line:column` entries, so `zed -- <repo root> <file>:<line>` opens the project and the file in one call.
  the root is passed only where a preset asks for it, never by default. the jetbrains launchers take a single path and have no such form, and passing them the repo root makes them write an `.idea` directory into the user's repo — so they keep the file alone and open it in LightEdit mode when it is outside an already-open project. terminal editors have no notion of a project window at all, so the root would just be a second file to open.
