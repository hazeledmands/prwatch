...
- when the file has a diff, derived from the visible viewport range against the file's hunk list:
  ...
  - a hunk counts as "visible" once at least two of its lines fit in the viewport (or, for a 1-line hunk like a pure deletion, its single line). a hunk whose only visible line is at the viewport's very top or bottom edge isn't enough — the rest is off-screen and reporting it as a hunk being viewed misleads.
